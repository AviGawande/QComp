...
Rectangle {
    ...
    // Simple state property - change this to control rectangles
    property var rectStates: [
        { id: "R1", active: true, clicked: false },
        { id: "R2", active: false, clicked: false },
        { id: "R3", active: true, clicked: false },
        { id: "R4", active: false, clicked: false },
        { id: "R5", active: true, clicked: false },
        { id: "R6", active: true, clicked: false }
    ]

    // Function to calculate position for each rectangle
    function getRectPosition(index) {
        var col = index % 2  // 0 or 1 (2 columns)
        var row = Math.floor(index / 2)  // 0, 1, or 2 (3 rows)
        var x = spacing + col * (rectWidth + spacing)
        var y = spacing + row * (rectHeight + spacing)
        return {x: x, y: y}
    }

    // Function to handle rectangle click
    function onRectClick(index) {
        if (rectStates[index].active) {
            rectStates[index].clicked = !rectStates[index].clicked
            rectStatesChanged()
        }
    }

    // Repeater to create rectangles
    Repeater {
        model: 6
        ...
        Rectangle {
            width: rectWidth
            height: rectHeight
            border.color: rectStates[index].active ? "black" : "gray"
            border.width: 2
            x: getRectPosition(index).x
            y: getRectPosition(index).y

            opacity: {
                if (!rectStates[index].active) return 0.3
                if (rectStates[index].clicked) return 0.7
                return 1.0
            }
            ...
            gradient: Gradient {
                GradientStop {
                    position: 0.0
                    color: rectStates[index].active ? "black" : "#404040"
                }
                GradientStop {
                    position: 1.0
                    color: rectStates[index].active ? "blue" : "#606060"
                }
            }

            Text {
                text: rectStates[index].id
                anchors.centerIn: parent
                font.pixelSize: 14
                font.bold: true
                color: "white"
            ...
            MouseArea {
                anchors.fill: parent
                enabled: rectStates[index].active
                onClicked: root.onRectClick(index)
            }
